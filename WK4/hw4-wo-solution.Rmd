...
#drop factors
machine.cont <- subset(machine.raw, select=-c(vendor,model))

#Log-transform all variables, and impute missing to 1
machine.log <- log(machine.cont + 1)

#Drop unnecessary variables
machine.data <- subset(machine.log, select=-c(ERP))

#Numerical summary
summary(machine.data)

#Scatter plot matrix
pairs(~., data = machine.data, main="Log tranformed X and Y")

#Box plot for each variable
boxplot(machine.data, main="Log transformed X and Y")



orderByCorrelation <- function(indata, outcome="PRP"){
  #Correlation with outcome 
  cor.out <- cor(indata[,outcome],indata)
  
  #Order by absolute value of correlation
  cor.ord <- order(-abs(cor.out))
  
  #Rearrange columns, from left to right with decreasing
  #correlation to outcome
  indata[,c(cor.ord)]  
  
}

machine.ord <- orderByCorrelation(machine.data)
...
quadProd <- function(xTmp) {
  ...
  tmpCnms <- NULL
  # for each linear term:
  # generate pairwise products for all predictors
  for ( iTmp in 2:dim(xTmp)[2] ) {
    for ( jTmp in iTmp:dim(xTmp)[2] ) {
      x2Tmp <- cbind(x2Tmp,xTmp[,iTmp]*xTmp[,jTmp])
      # maintain vector of column names for quadratic
      # terms along the way:
  ...
  # return data.frame with outcome as a first column,
  # followed by linear, then by quadratic terms:
  data.frame(xTmp,x2Tmp)
}

machine.quad <- quadProd(machine.ord)

# Re-order predictors by correlation to outcome 
machine.quad <- orderByCorrelation(machine.quad)
...
plotRegErrRes <- function(indata){
  df2plot <- NULL
  for ( iTmp in 2:dim(indata)[2] ) {
    lmTmp <- lm(Y~.,indata[,1:iTmp])
    errTmp <- sqrt(mean((indata[,"Y"]-predict(lmTmp))^2))
    df2plot <- rbind(df2plot,data.frame(nvars=iTmp-1,err=errTmp))
  }
  plot(df2plot,xlab="Number of variables",ylab="Regression error",main=paste(dim(machine.quad)[1],"observations"))
}

# Rename outcome variable to Y to make it consistent with bootTrainTestErrOneAllVars
names(machine.quad)[1] <- "Y"
plotRegErrRes(machine.quad)


#For the data presented in the preface, we see a sharp decline in the regression error 
# after the addition of the 3rd predictor, around the 16th predictor, we see minimal effect on the regression error

# Conversely for the example here we see a large drop in regression error after the second variable. It seems that the interaction term of MMAX and MMIN is the strongest predictor of PRP (performance) in our model.As MMIN and MMAX are ranges of min and max memory, we would expect these to be highly correlated. 
# The second most important predictor of performance was the interaction of Cach memory and max channels in units

cor(machine.data$MMIN,machine.data$MMAX)
plot(machine.data$MMIN,machine.data$MMAX)
```
...
machine.booterr <- bootTrainTestErrOneAllVars(machine.quad,30)

#Not sure if anything else is expected here. Renaming the outcome variable seems sufficient to me?
```
...
plotBootRegrErrRes(machine.booterr,mainTxt="Outcome: PRP, 208 observations")

# As observed previously, there is a largest drop in regression error is explained by the first two predictors (most correlated with the performance)
# As expected the test error for the bootstrap is higher than the training, likely due to overfitting particularly as we add more variables and consequently noise to the model.

# Saving a copy of the errors when using PRP as the outcome
machine.prp <- machine.booterr


# Redo analysis using ERP as the outcome instead of PRP
machine.erp <- subset(machine.log, select = -c(PRP))

#Order by correlation to outcome
machine.erp.ord <- orderByCorrelation(machine.erp,outcome = "ERP")

#Adding quadratic terms
machine.erp.quad <- quadProd(machine.erp.ord)

#Re-order predictors by correlation to outcome 
machine.erp.quad <- orderByCorrelation(machine.erp.quad, outcome = "ERP")


#Regression error using all data as training
names(machine.erp.quad)[1] <- "Y"
plotRegErrRes(machine.erp.quad)


#Bootstrapped regression error, split on training and test for ERP
machine.erp.booterr <- bootTrainTestErrOneAllVars(machine.erp.quad,30)
plotBootRegrErrRes(machine.erp.booterr,mainTxt="Outcome: ERP, 208 observations")


#For predicting ERP, we need to 4 predictors to reduce most of the regression error
#The test bootstrap regression error oscillates after 4 predictors are used in the model, however it doesnt overfit as much as the PRP model
